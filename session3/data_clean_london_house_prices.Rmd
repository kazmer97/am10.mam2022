...
london_postcodes <-  read_csv(here::here("data","london_postcodes.csv")) %>% 
    janitor::clean_names()
...
london_matched <- london_matched %>% 
  left_join(london_postcodes) 

colnames(london_matched)
...
london_matched <- london_matched %>% # most expensive house ever sold is 200M in London
  filter(price < 200*10^6)
...
cut_houses <- expensive_houses %>% 
  filter(is.na(postcode) | str_detect(address2, "GROUND" ))


london_matched <- london_matched %>% 
  filter(!id %in% cut_houses$id) %>% 
  filter(!id %in% cheap_houses$id)
...
write_csv(london_matched, "clean_london_house_prices.csv")
